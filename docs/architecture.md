...
**R-backed baselines** (Monocle3, Slingshot-R) are designed to run as isolated subprocesses via `python -m litchron.baselines._r_runner <method> <run_id> <h5ad_path>`. The subprocess loads h5ad, calls rpy2, emits a single JSON line on stdout, then exits. If the R session were to segfault (Monocle3's `learn_graph` is a known offender on degenerate UMAP inputs), or if R's allocator raised an `R_MAX_VSIZE` abort — both of which bypass Python `try/except` — the subprocess dies but the MCP server process survives. The wrappers (`litchron/baselines/monocle3.py`, `slingshot_r.py`) capture stderr and return a structured `BaselineFailure` with reason code to the LLM.

> **Implementation status (as of this revision):** the real rpy2 dispatch inside `litchron/baselines/_r_runner.py` is a placeholder — it returns the `monocle3_not_implemented` / `slingshot_r_not_implemented` error codes immediately. A deterministic linear-pseudotime stub is available by setting `LITCHRON_STUB_R=1` in the environment; the stub exercises the full subprocess plumbing (argv, JSON-on-stdout, exit codes, `BaselineFailure` wrapping) without importing rpy2. Real Monocle3 and Slingshot-R runs are tracked in [issue #5](https://github.com/PeterPonyu/litchron/issues/5).

**Dispatch rule** in `run_baseline`: `if method in R_BACKED: subprocess else: in-process`. The R-vs-Python partition (`R_BACKED`, `PYTHON_BACKED`) and the `run(method, ...)` dispatcher itself both live in `litchron/baselines/__init__.py`; there is no separate `registry.py` module.
