"""Monocle3 baseline wrapper (subprocess-isolated).

This module **never** imports :mod:`rpy2`. Instead it spawns
:mod:`litchron.baselines._r_runner` as a subprocess, parses the
single-line JSON contract, persists artifacts in the standard layout,
and surfaces failures (including SIGSEGV) as
:class:`mcp_litchron.errors.BaselineFailure`.
"""
...
def _parse_ordering_csv(path: Path) -> tuple[list[str], np.ndarray]:
    """Parse the runner's ordering.csv. Returns (cell_ids, pseudotime)."""
    cell_ids: list[str] = []
    pst: list[float] = []
...
def _save_plot(pseudotime: np.ndarray, out_path: Path) -> None:
    ...
    ax.set_title("monocle3 pseudotime (sorted)")
...
def _write_delta_zarr(out_path: Path, pseudotime: np.ndarray) -> None:
    ...
    obs.create_dataset("monocle3_pseudotime", data=arr, shape=arr.shape, dtype="f8")


def run_monocle3(run_id: str, h5ad_path: str, run_dir: Path) -> BaselineResult:
    """Spawn the R subprocess runner and persist Monocle3 baseline artifacts."""
    ...
    log_lines: list[str] = [f"[{_now()}] monocle3.run_monocle3 start run_id={run_id}"]
    ...
    try:
        ...
        raise BaselineFailure(
            code="monocle3_timeout",
            message=f"Monocle3 subprocess exceeded {_TIMEOUT_S}s",
    ...
    # SIGSEGV (POSIX) -- subprocess reports negative signal as -11, or
    # 139 when the shell reports 128 + signal. Treat both as segfault.
    if proc.returncode in (-11, 139):
        ...
        raise BaselineFailure(
            code="segfault",
            message="Monocle3 subprocess crashed (SIGSEGV in libR).",
            hint=(
                "Re-run; if reproducible, downgrade Monocle3 / Bioconductor "
                "or report upstream."
            ),
            retryable=True,
            method=_METHOD,
    ...
    # Parse stdout JSON (single line contract).
    ...
    if payload is None:
        try:
            ...
            pass
        raise BaselineFailure(
            code="monocle3_no_json",
            message="Monocle3 runner produced no JSON status line on stdout.",
    ...
    if payload.get("status") != "ok" or proc.returncode != 0:
        try:
            ...
            pass
        raise BaselineFailure(
            code=str(payload.get("code") or "monocle3_error"),
            message=str(
                payload.get("message") or "Monocle3 runner reported non-ok status."
            ),
    ...
    if not ordering_csv:
        raise BaselineFailure(
            code="monocle3_missing_ordering",
            message="Monocle3 runner did not report an ordering_csv path.",
    ...
    try:
        ...
        log_lines.append(f"[{_now()}] monocle3.run_monocle3 done")
        log_path.write_text("\n".join(log_lines) + "\n", encoding="utf-8")
        ...
        return BaselineResult(
            ...
            adata_delta_keys=["obs/monocle3_pseudotime"],
        )
    except BaselineFailure:
        raise
    except Exception as exc:  # noqa: BLE001
        log_lines.append(f"[{_now()}] FAIL post-runner {type(exc).__name__}: {exc}")
        try:
            log_path.write_text("\n".join(log_lines) + "\n", encoding="utf-8")
        except OSError:
            pass
        raise BaselineFailure(
            code="monocle3_persist_error",
            message=(
                f"Failed to persist Monocle3 artifacts: {type(exc).__name__}: {exc}"
            ),
